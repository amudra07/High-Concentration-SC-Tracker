...
def topbar(show_home: bool):
    ...
    st.markdown(f"""
    <div class="topbar">
        <span>SC TECH TRACKER &middot; last updated {LAST_UPDATED.isoformat()}</span>
        {home_html}
    </div>
    """, unsafe_allow_html=True)
...
def render_home():
    ...
    st.markdown('<h1 class="page-title">Four paths to high-concentration SC delivery — Positioning of our technology</h1>', unsafe_allow_html=True)
...
def render_landscape():
    ...
    with k3:
        st.markdown(f'<div class="kpi"><div class="v">{top["concentration_mgml"]}+</div><div class="l">Targeted {top["name"]} Concentration</div></div>', unsafe_allow_html=True)
    with k4:
